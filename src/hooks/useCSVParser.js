...
export const useCSVParser = (setParticipants, setAlerts) => {
  const parseCSVData = useCallback((csvText) => {
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      headers.forEach((header, index) => {
        ...
        else if (headerLower.includes("preference")) row.preference = value || "No Preference";
        else if (headerLower.includes("half") && headerLower.includes("round")) row.halfRound = value || "";
        else if (headerLower === "role") row.role = value || "Debate";
      });

      if (row.experience === "General Member") row.experience = "General Members";
      ...
      } else if (row.name) skipped.push(`${row.name} (missing experience level)`);
    }
  ...
  const handleCSVInput = useCallback((csvText) => {
    if (!csvText?.trim()) {
    ...
    try {
      ...
      if (data.length === 0) {
        setAlerts([{
          type: "error",
          message: "No valid data found in CSV. Check column headers and experience levels."
        }]);
        return;
      }
      ...
      setAlerts([{ type: "error", message: "Failed to parse CSV: " + error.message }]);
    }
